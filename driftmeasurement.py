...
def main() -> None:
    ...
    MASKS_DIR  = "/scr/jpember/weighted_polly_outputs/masks"
    OUTPUT_DIR = "/scr/jpember/weighted_polly_outputs/"
    ...
    for reference_wavelength, local_spacing in\
                                zip(reference_wavelengths, local_spacings):
        ...
        d = PeakDrift(
                ...
                drift_file = f"{OUTPUT_DIR}/"+\
                    f"raw_drifts_{ref_date:%Y%m%d}_{max_date:%Y%m%d}/"+\
                                            f"{reference_wavelength:.2f}.txt",
                force_recalculate = False,
            )
        if len(d.valid_wavelengths) <= 3:
            continue
    ...
    for d in drifts:
        ...
        if d.reference_wavelength <= 4897\
            or 5998 <= d.reference_wavelength <= 6044:
            continue
        ...
        plt.scatter(
            ts,
            constants.c.to(u.m / u.s) * d.deltas / d.reference_wavelength,
            s = 50,
        ...
        # plt.plot(
        #     [t0, t1],
        #     constants.c.to(u.cm / u.s) * d.fit([t0, t1]),
