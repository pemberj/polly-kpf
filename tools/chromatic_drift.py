...
def RV_drift(
    ...
    average_sigmas = 1 / np.sqrt(np.sum(1 / rv_df[sigma_columns] ** 2, axis=0))

    wavelengths = [float(wl) for wl in wl_columns]

    return (
        Quantity(wavelengths, unit=u.angstrom),
...
def main() -> None:
    ...
    wavelengths, rvs, sigmas = RV_drift(
    ...
    ax.scatter(
        x=wavelengths,
        y=rvs,
        color=wavelength_to_rgb(wavelengths.value),
        s=30,
        alpha=0.75,
        zorder=10,
    )
    ...
    ax.set_ylabel("Average RV Drift (m/s)")

    ax.set_xlim(4900, 8800)
    ax.set_ylim(-1, 12)

    plt.savefig(
        DRIFTS_DIR / "multi_RV_drift_chromatic.png",
        dpi=300,
        bbox_inches="tight",
    )
